 ...
 *
 * Same draw as the classic screen — it runs the identical `useAdminDraw` hook,
 * so spins, winner recording and the broadcast to the public view-only page
 * behave exactly the same. Only the painting differs.
 *
 * `defaultVariant` is what a bare URL renders; `?ui=` overrides it. Both admin
 * routes render this component, differing only in that default.
 */
export default function PixelAdminScreen({
  defaultVariant,
}: {
  defaultVariant: PixelVariant;
}) {
  const params = useParams();
  const searchParams = useSearchParams();
  const variant =
    PIXEL_VARIANTS[searchParams.get("ui") ?? ""] ?? defaultVariant;
  const buttonStyle = {
